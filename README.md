...
- `80` : Service가 내부에서 받는 포트
- `30500` : 노드 IP에서 외부에 개방된 포트
- `TCP` : 프로토콜

이 구조는 다음과 같이 정리됩니다.

```
노드IP:NodePort → ClusterIP:ServicePort → Pod IP:targetPort
```
...
```
목적지 10.99.96.62:80 → 실제 Pod IP:80
```
...
- nginx Service : `10.99.96.62:80`(ClusterIP)
- 외부 접근 포트 : `30500`(NodePort)
...
- `NodePort` : 외부에서 노드 IP로 접속할 때 사용하는 포트
- `ClusterIP` : 클러스터 내부에서 사용하는 가상 IP
